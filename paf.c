...
int main(){
	...
	p=&array[0];
	for(i=0; i<N;i++){									      //Pasamos los caracteres del archivo (ascii) a enteros.
            if(48 < *p && *p < 58){
            	array2[j]= *p -48;						      //Los guardamos en un arreglo entero.
            	printf("%d",array2[j]);
            	j++;
            }
            *p++;
    ...
    pu=&array2[0];
    puntero = &ext[1];
    for (k = 1; k< N; ++k){								      //Comenzamos en la primera operacion.
        suma1 = *puntero + suma1;
    	*puntero++;
    	if (k==4){
    		suma1 = abs(*pu - suma1);					      //Restamos las sumas y obtenemos la diferencia absoluta.
    	}
    }
    pu=&array2[0];										      //Posicion de punteros "reiniciada" a comodidad.
    puntero=&ext[2];
    for (k = 2; k< 5; ++k){								      //Hacemos la segunda operacion.
        suma2 = *puntero + suma2;
    	*puntero++;
    	if (k==4){
    		for (i = 0; i < 2; ++i){
   			...
   			suma2 = abs(suma5 - suma2);					      //Restamos las sumas y obtenemos la diferencia absoluta.
    	}
    }
    pu=&array2[0];										      //Posicion de punteros "reiniciada" a comodidad.
    puntero=&ext[3];
    suma5=0;
    for (k = 3; k < 5; ++k){							      //Hacemos la tercera operacion.
    	suma3 = *puntero + suma3;
    	*puntero++;
    	if (k==4){
    		for (i = 0; i < 3; ++i){
    		...
    		suma3 = abs(suma5 - suma3);					      //Restamos las sumas y obtenemos la diferencia absoluta.
    	}
    }
    pu=&array2[0];
    puntero=&ext[4];									       //Posicion de punteros "reiniciada" a comodidad.
    suma5=0;
    for (k = 4; k < 5; ++k){
    	suma4 = *puntero + suma4;
    	if (k==4){
    		...
    		suma4= abs(suma5 - suma4);                          //Restamos las sumas y obtenemos la diferencia absoluta.
    	}
	}
	if (suma1 < suma2 && suma1 < suma3 && suma1 < suma4){	    //Buscamos cual es la diferencia absoluta menor.
		minimadif = suma1;									    //Y igualamos la minima diferencia a la menor ya sea el caso.
	}
